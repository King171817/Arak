...
String tr(AppLang lang, String key) {
  final Map<String, Map<String, String>> data = {
    ...
    'login_info': {
      'FA': 'دانشجو: admin / مدیران: admin1 تا admin4 / استاد: prof1, prof2 / رمز: 1234',
      'EN': 'Student: admin / Managers: admin1 to admin4 / Professor: prof1, prof2 / Password: 1234',
      'AR': 'الطالب: admin / المدراء: admin1 إلى admin4 / الأستاذ: prof1, prof2 / كلمة المرور: 1234',
    },
...
class AppState extends ChangeNotifier {
  ...
  String _userRole = 'guest'; // 'guest', 'student', 'manager', 'professor'
  String _userIdentifier = ''; // student ID, manager unitKey, professor ID
  bool _isDarkMode = false;
  bool _isLoggedIn = false;
  
  // اضافه کردن لیست اعلان‌ها برای نقش‌های مختلف
  List<AppNotification> _managerNotifications = [];
  List<AppNotification> _professorNotifications = [];
  List<AppNotification> _studentNotifications = [];
  ...
  // Getter برای اعلان‌ها
  List<AppNotification> get managerNotifications => _managerNotifications;
  List<AppNotification> get professorNotifications => _professorNotifications;
  List<AppNotification> get studentNotifications => _studentNotifications;
  ...
  void login(String role, String identifier) {
    ...
    // بارگذاری اعلان‌های مربوط به نقش هنگام ورود
  ...
  // اضافه کردن اعلان جدید برای نقش خاص
  ...
  // دریافت اعلان‌ها بر اساس نقش فعلی
  ...
  // دریافت تعداد اعلان‌های نخوانده برای نقش فعلی
  int getUnreadNotificationCountForCurrentRole() {
    final notifications = getCurrentRoleNotifications();
    return notifications.where((n) => n.unread).length;
  }
  
  // علامت زدن اعلان به عنوان خوانده شده
  void markNotificationAsRead(String notificationId, String role) {
    ...
    final index = targetList.indexWhere((n) => n.title == notificationId);
    if (index != -1) {
      ...
      targetList[index] = updatedNotification;
      notifyListeners();
    }
  }
  
  // بارگذاری اعلان‌های نمونه برای هر نقش
    ...
    switch (role) {
      case 'manager':
        _managerNotifications = [
          ...
          AppNotification(
            title: 'گزارش ماهانه',
            subtitle: 'گزارش عملکرد مهرماه آماده است.',
      ...
      case 'professor':
        _professorNotifications = [
          AppNotification(
            ...
            subtitle: 'فردا کلاس لغو شده است.',
            unitKey: 'education',
            unread: true,
          ),
          AppNotification(
            title: 'جلسه اساتید',
            subtitle: 'جلسه هماهنگی اساتید سه‌شنبه برگزار می‌شود.',
            unitKey: 'education',
            unread: true,
          ),
          AppNotification(
            title: 'درخواست دانشجو',
            subtitle: 'دانشجو سوالی درباره پروژه دارد.',
            unitKey: 'student_services',
            unread: false,
          ),
        ];
      ...
      case 'student':
        _studentNotifications = [
          ...
          AppNotification(
            title: 'اطلاعیه خوابگاه',
            subtitle: 'زمان تحویل خوابگاه اعلام شد.',
            unitKey: 'student_services',
            unread: false,
          ),
        ];
  ...
  // اضافه کردن پیام جدید بین مدیران
  void addManagerMessage(String targetUnit, String message) {
    final newNotification = AppNotification(
      title: 'پیام جدید از ${_getUnitName(_userIdentifier)}',
      subtitle: message.length > 50 ? '${message.substring(0, 50)}...' : message,
      unitKey: targetUnit,
      unread: true,
    );
    
    // اضافه کردن به اعلان‌های مدیر مقصد
    addNotificationForRole('manager', newNotification);
  }
  
  // دریافت نام واحد به زبان فارسی
  String _getUnitName(String unitKey) {
    switch (unitKey) {
      case 'international': return 'امور بین‌الملل';
      case 'education': return 'آموزش';
      case 'student_services': return 'خدمات دانشجویی';
      case 'consular': return 'کنسولی';
  ...
  // ارسال پیام از استاد به مدیر واحد
  ...
  void sendMessageToProfessor(String professorId, String message, String unitName) {
    ...
    addNotificationForRole('professor', newNotification);
  }
}
            ...
            SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 80), // فضا برای دکمه شناور
                ],
              ),
            ),
...
class HeaderWithInteractiveSidePanel extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (activePanel == 'notifications' && appNotifications.length > 3) {
...
class ProfessorHomeScreen extends StatefulWidget {
  ...
  const ProfessorHomeScreen({super.key, required this.professorId});
...
class _ProfessorHomeScreenState extends State<ProfessorHomeScreen> {
  ...
  Widget _buildBody() {
    switch (_selectedBottomNavIndex) {
      ...
      case 1:
        return _buildMyClasses();
      case 2:
        return _buildCommunicationWithUnits();
      default:
  ...
  Widget _buildDashboard() {
    ...
    final myClasses = mockClasses.where((c) => professor.classIds.contains(c.id)).toList();
    final totalStudents = myClasses.fold(0, (sum, c) => sum + c.studentIds.length);
      ...
      children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      '${isRtl ? 'تعداد کلاس‌ها' : 'Classes'}: ${myClasses.length}',
        ...
        Row(
          children: [
            ...
            Expanded(
              child: _StatCard(
                title: isRtl ? 'تعداد دانشجویان' : 'Total Students',
                value: '$totalStudents',
                icon: Icons.people,
                color: Colors.green,
              ),
        ...
        // اطلاعیه‌های آموزشی
        Text(
          isRtl ? 'اطلاعیه‌های آموزشی' : 'Educational Announcements',
          style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 17),
        ),
        const SizedBox(height: 12),
        Container(
          ...
          child: Column(
            children: _getProfessorAnnouncements().map((announcement) {
  ...
  Widget _buildMyClasses() {
    ...
    return Column(
      ...
      children: [
        Text(
          isRtl ? 'کلاس‌های من' : 'My Classes',
          style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 17),
        ),
        const SizedBox(height: 12),
        ...myClasses.map((classModel) => _ProfessorClassCard(
          classModel: classModel,
          onTap: () {
            Navigator.push(
              context,
              MaterialPageRoute(
                builder: (context) => ProfessorClassScreen(classModel: classModel),
              ),
            );
          },
        )),
  ...
  // ارتباط با واحدها
  Widget _buildCommunicationWithUnits() {
    ...
    return Column(
      ...
      children: [
        Text(
          isRtl ? 'ارتباط با واحدهای دانشگاه' : 'Communication with University Units',
          style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 17),
        ),
        const SizedBox(height: 12),
        ...units.where((u) => u.keyName != 'other_services').map((unit) {
          return _UnitChatCard(
            unit: unit,
            onTap: () {
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (context) => ProfessorUnitChatScreen(
                    unitKey: unit.keyName,
                    unitName: appText(selectedLang, unit.keyName),
  ...
  void _openOtherService(String serviceKey) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => OtherServiceScreen(serviceKey: serviceKey),
      ),
    );
  }
}
...
// کارت کلاس استاد
class _ProfessorClassCard extends StatelessWidget {
  final ClassModel classModel;
  final VoidCallback onTap;

  const _ProfessorClassCard({
...
// کارت ارتباط با واحد
class _UnitChatCard extends StatelessWidget {
  final UnitModel unit;
  final VoidCallback onTap;
  ...
  Widget build(BuildContext context) {
    ...
    return Card(
      ...
      child: ListTile(
        ...
        title: Text(appText(selectedLang, unit.keyName)),
        subtitle: const Text('ارسال پیام به مدیر واحد'),
        trailing: const Icon(Icons.chat_outlined, color: Colors.green),
        onTap: onTap,
      ),
    );
  }
...
// تابع دریافت اطلاعیه‌های استاد
List<Map<String, String>> _getProfessorAnnouncements() {
  return [
    {
      'title': 'تمدید مهلت ثبت نام',
      'body': 'مهلت ثبت نام تا 30 مهر ماه تمدید شد.',
      'date': '1403/07/15',
    },
    {
      'title': 'جلسه هماهنگی اساتید',
      'body': 'جلسه هماهنگی اساتید روز سه شنبه برگزار می‌شود.',
      'date': '1403/07/20',
    },
  ];
}
  ...
  List<ClassReportModel> _generateClassReports() {
    ...
    for (var classItem in _managedClasses.where((c) => c.status == 'completed')) {
      ...
      reports.add(ClassReportModel(
        ...
        attendanceRate: attendanceRateValue.toDouble(),  // تبدیل به double
      ));
    }
    return reports;
  }
