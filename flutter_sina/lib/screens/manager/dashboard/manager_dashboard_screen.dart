    ...
    return Container(
      ...
      child: ListView(
        ...
        children: <Widget>[
          ...
          _ManagerLiveSessionsMonitor(rtl: rtl),
...
/// نظارت بر جلسات زندهٔ در حال اجرا (استاد شروع/پایان می‌زند).
class _ManagerLiveSessionsMonitor extends StatelessWidget {
  final bool rtl;

  const _ManagerLiveSessionsMonitor({required this.rtl});

  @override
  Widget build(BuildContext context) {
    final LiveClassState live = context.watch<LiveClassState>();
    final List<LiveSessionModel> active = live.sessions
        .where((LiveSessionModel s) => s.status == 'active')
    ...
    return Container(
      ...
      child: ExpansionTile(
        ...
        title: Text(
          rtl
              ? 'کلاس زنده فعال (${active.length})'
              : 'Live sessions (${active.length})',
          style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 13),
        ),
        subtitle: Text(
          rtl
              ? 'شروع و پایان با استاد؛ نظارت توسط مدیریت آموزش'
              : 'Start/end by professor; oversight by education admin',
          style: const TextStyle(fontSize: 10),
        ),
        children: active.map((LiveSessionModel s) {
          final List<LiveSessionParticipantModel> parts =
              live.participantsOf(s.id);
          final int online =
              parts.where((LiveSessionParticipantModel p) => p.isOnline).length;
          final int present =
              parts.where((LiveSessionParticipantModel p) => p.present).length;

          return ListTile(
            dense: true,
            title: Text(s.title),
            subtitle: Text(
              '${s.professorName} · آنلاین: $online · حضور ثبت‌شده: $present',
              style: const TextStyle(fontSize: 10),
            ),
          );
        }).toList(),
